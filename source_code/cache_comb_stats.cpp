...
void print_stats() {
	...
	float total_hits  = float(statistics.data_hit)+ float (statistics.inst_hit);
	float total_misses = float(statistics.data_miss) + + float(statistics.inst_miss);
	float total_hits_misses = float(total_hits) + float (total_misses);


	float total_hit_r = float(total_hits) / (float(total_hits) + float(total_misses));
	...
	if (total_misses == 0) {	// If we don't have any misses, then no operations took place!
	...
	cout << "  Cache Hit Ratio: \t\t" << dec << total_hit_r << endl;
	cout << "  Cache Hit Percentage: \t" << dec << (total_hit_r) * 100 << " %\n\n" << endl;
