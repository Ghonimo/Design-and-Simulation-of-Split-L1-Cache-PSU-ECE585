...
int main(int argc, char** argv) {
	...
	// call the import function => import(filename); (or make it a void if we import/read the file in it)
	import(trace_file);
...
void import(char* filename) {
	...
	char trace_address[1024];							// the address is 8 hexvalues = 8*(4bits) 	
	...
	fp = fopen(filename, "r");							// "r" switch of the fopen function opens a file for reading

	while (fgets(trace_address, sizeof trace_address, fp)) {

		sscanf(trace_address, "%c %x", trace_operation, &address);	// %c is a swtich for a single character, %x is for hexacdecimal in lowercase letters
		if (trace_address[0] == '#' || !strcmp(trace_address, "\n") || !strcmp(trace_address, " ")) {
		}
		else {
...
void fetch_inst(unsigned int addr) {
	...
	if (way >= 0) {										// if we have a matching tag, then we have an instruction cache hit! (unless invalid MESI state)
		...
		if (way >= 0) {								// if we have an empty line, place the fetched instruction in it
			...
			if (mode == 1) {						// Simulating the cache fetch instruction communication with L2
				cout << " [Instruction-Operation] Read from L2: L1 cache miss, obtain instruction from L2 " << hex << addr << endl;
			}
		}
...
void print_stats() {
	...
	if (isnan(data_hit_r)) {	//determine if float number is not-a-number (NaN) value
		data_hit_r = 0;			//if NaN, set equal to 0
	}
	if (isnan(inst_hit_r)) {	//this way we can properly display stats without a value
		inst_hit_r = 0;
	}
	...
	int both_miss = statistics.data_miss + statistics.inst_miss;

	cout << "\n\t -- COMBINED CACHE STATISTICS -- " << endl;

	if (both_miss == 0) {	// If we don't have any misses, then no operations took place!
		cout << "  The cache was not used/not operated on." << endl;
	}
	
	//below should print out every time this function is ran, or when '9' is sensed
	cout << "  Number of Cache Reads: \t" << dec << statistics.inst_read + statistics.data_read << endl;
	cout << "  Number of Cache Writes: \t" << dec << statistics.data_write << endl;
	cout << "  Number of Cache Hits: \t" << dec << statistics.inst_hit + statistics.data_hit << endl;
	cout << "  Number of Cache Misses: \t" << dec << statistics.inst_miss + statistics.data_miss << endl;
	cout << "  Cache Hit Ratio: \t\t" << dec << statistics.inst_hit_ratio + statistics.data_hit_ratio << endl;
	cout << "  Cache Hit Percentage: \t" << dec << (statistics.inst_hit_ratio + statistics.data_hit_ratio) * 100 << " %\n\n" << endl;
